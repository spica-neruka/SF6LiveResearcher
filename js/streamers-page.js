(() => {
  ...
  const state = { page: 0, sort: 'newest', category: 'all', total: 0, loading: false, lastKey: '', favoriteKey: '' };
  ...
  async function fetchAllFavoriteStreamers(favorites) {
    const found = new Map(); let offset = 0; let total = 0;
    do {
      ...
      for (const streamer of data.items || []) if (favorites.includes(streamer.channel_id)) found.set(streamer.channel_id, streamer);
      offset += PAGE_SIZE;
  ...
  async function renderOfflineFavorites() {
    ...
    let favorites = []; try { favorites = JSON.parse(localStorage.getItem('sf6-live-favorites') || '[]'); } catch {}
    const favoriteKey = favorites.join('|');
    if (state.favoriteKey === favoriteKey && app.dataset.fullFavorites === '1') return;
    state.favoriteKey = favoriteKey;
    if (!favorites.length) {
      app.dataset.fullFavorites = '1';
    ...
    try {
      const [streamers, liveResponse] = await Promise.all([fetchAllFavoriteStreamers(favorites), fetch(`${API_BASE}/api/videos?status=live&limit=200`, { cache: 'no-store' })]);
      ...
      const items = favorites.map(channelId => { const streamer = streamers.get(channelId); return streamer ? { streamer, live: liveByChannel.get(channelId) || null } : null; }).filter(Boolean);
      app.dataset.fullFavorites = '1';
      app.innerHTML = `<section class="section"><div class="section-head"><div><h2>お気に入り</h2><p>登録した配信者を、配信中でなくても表示します。</p></div><span class="result-count">${items.length} streamers</span></div><div class="streamer-grid">${items.map(({ streamer, live }) => { const name = streamer.sf6_player_name || streamer.channel_title || streamer.channel_id; return `<article class="streamer-card" data-favorite-channel="${escapeHtml(streamer.channel_id)}" data-live-video="${escapeHtml(live?.video_id || '')}"><div class="channel-avatar-wrap">${icon(streamer.channel_thumbnail_url, name)}</div><div class="streamer-row"><strong>${escapeHtml(name)}</strong><button class="favorite on" data-favorite-remove="${escapeHtml(streamer.channel_id)}" aria-label="お気に入りから削除">♥</button></div><div class="streamer-stats"><span><b>登録者</b>${streamer.subscriber_count != null ? Number(streamer.subscriber_count).toLocaleString() : '--'}</span><span><b>カテゴリ</b>${escapeHtml(label(streamer.streamer_category))}</span></div><small>${live ? '🔴 現在配信中' : 'YouTubeチャンネル'}</small></article>`; }).join('')}</div></section>`;
  ...
  document.addEventListener('click', (event) => {
    if (isFavoritesPage() && event.target.closest('[data-favorite-remove]')) {
      event.stopPropagation(); const id = event.target.closest('[data-favorite-remove]').dataset.favoriteRemove;
      let favorites = []; try { favorites = JSON.parse(localStorage.getItem('sf6-live-favorites') || '[]'); } catch {}
      localStorage.setItem('sf6-live-favorites', JSON.stringify(favorites.filter(x => x !== id))); state.favoriteKey = ''; renderOfflineFavorites(); return;
    }
  ...
  const observer = new MutationObserver(() => {
    ...
    else { state.lastKey = ''; state.favoriteKey = ''; }
  });
